...
psych::fa.parallel(na.omit(proc_data), fa = "pc", n.iter = 500)
```
...
```
::: {.callout-note}
El índice saca contruido, se construyo multiplicando las cargas por las variables en su escala original, 
pero las cargas provienen de un PCA sobre variables estandarizadas.
Para realizarlo de aquella forma o conciderandondo el PCA sobre las variables estandarizadas, se puede realizar el siguiente codigo.
:::

```{r, eval=FALSE}

fit <- FactoMineR::PCA(na.omit(proc_data), scale.unit = TRUE, graph = FALSE)
indice <- fit$ind$coord[, 1]
